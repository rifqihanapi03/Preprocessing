...
#thresholding
_, thr1 = cv2.threshold(grayImage,127, 255, cv2.THRESH_BINARY_INV)
...
#dilate
#dilate = cv2.dilate(thr2, None, iterations=2)

#Segmentasi
edges = cv2.Canny(thr1,100,200)

#FCC
#contours,hierarchy = cv2.findContours(thr2, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
#print ("Number of contours = " + str(len(contours)))
#print (contours[0])
#cv2.putText(image, "Number of contours = {}".format(len(contours))), (400,35), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0),2)

#cv2.drawContours(grayImage, contours,15, (0,255,0),3)
...
cv2.imshow('gambar grayscale',thr1)
cv2.imshow('gambar thresholding',edges)

#np.savetxt('matrix gray.txt',np.array(grayImage),fmt="%s")
cv2.waitKey(0)
